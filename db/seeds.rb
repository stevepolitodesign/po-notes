...
User.all.each do |user|
  ...
  1.upto(100) do |i|
    pinned = i == 1
    public = i == 1
    title = i == 1 ? "Markdown Demo" : Faker::Lorem.sentence
    body = i == 1 ? markdown : Faker::Lorem.paragraphs.join

    @note = user.notes.build(title: title, body: body, pinned: pinned, public: public)
    @note.save! if @note.valid?
    puts "Created Note: #{@note.title}" if @note.valid?
    @note.tag_list.add(tags.sample((1..tags.length).to_a.sample))
    @note.save!
